...
This extends a Browser Use agent; it is different from [running Browser Use as an MCP server](/open-source/customize/integrations/mcp-server).

Install [uv](https://docs.astral.sh/uv/getting-started/installation/) and Browser Use, and set `BROWSER_USE_API_KEY` for the agent's model.
Warm the reference time server's package cache before connecting:

```bash
uvx mcp-server-time==2026.8.18 --help
```
...
async def main():
    ...
    client = MCPClient(
        server_name="time",
        command="uvx",
        args=["mcp-server-time==2026.8.18", "--local-timezone=UTC"],
    )
    try:
        await client.register_to_tools(
            tools, tool_filter=["get_current_time"], prefix="time_"
        )
        agent = Agent(
            task="Use time_get_current_time to report the current UTC time.",
            llm=ChatBrowserUse(),
            tools=tools,
        )
        await agent.run()
