...
```
```bash curl
# first run returns { "session_id": "ses_xyz789", ... }
...
<Note>
The list filter query param is `?sessionId=` (**camelCase**), even though request bodies use snake_case `session_id`. Sending `?session_id=` is silently ignored and returns *all* your runs — use `sessionId`.
</Note>
