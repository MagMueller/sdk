...
| --- | --- | --- |
| **`browser-use`** | `result` string on the run | `output` field, typed — pass a `structured_output` schema (see [Structured output](/cloud/agent/structured-output)) |
...
<Note>
Attach a bad id and the run still starts (`202`) — the unknown ids come back in **`missing_file_ids`** on the create response. Always check that array; a missing file is not an error status.
</Note>
...
curl "https://api.browser-use.com/api/v1/agents/browsercode/runs/RUN_ID/files?kind=output" \
  -H "X-Browser-Use-API-Key: $BROWSER_USE_API_KEY"
# → { "items": [{ "id": "out_1", "name": "clean.csv", "kind": "output", "download_url": "https://..." }] }

# download_url is a presigned direct link — fetch it straight (no API endpoint, no key)
curl -L "https://.../presigned-download-url" -o clean.csv
```
</CodeGroup>

`download_url` is a **presigned, expiring link** you GET directly — it points straight at storage, not at a Browser Use endpoint. If it expires, re-list to get a fresh one.
...
<Note>
`browsercode`'s output is **always files** — a CSV, a report, a chart PNG, a schema'd `output.json`. Name the file in the task, fetch it with `?kind=output`. Only `browser-use` returns typed data inline via `structured_output`.
</Note>
...
The clean way: have `browser-use` return **typed data** (via `structured_output`), then feed it into `browsercode`'s task or attach it as a file.

<CodeGroup>
```python Python
# 1. browser-use scrapes → typed output
scrape = client.agents.browser_use.run(
    task="Scrape all product names and prices from this catalog",
    structured_output={"type": "object", "properties": {"products": {"type": "array"}}},
)
data = scrape.output   # typed dict
