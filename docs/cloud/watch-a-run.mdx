...
**`result` vs `output`:** `result` is the plain-language answer string, present on every completed run — read it on the golden path. `output` is `null` unless you requested [structured output](/cloud/agent/structured-output) (typed object, `browser-use` only). `browscode`'s larger output comes back as [files](/cloud/files), not in either field.
...
`browser_id` appears on the run once its browser is provisioned (null while queued). Use it to fetch the [recording or downloads](/cloud/manage-browser).

```bash curl
curl "https://api.browser-use.com/api/v1/agents/browser-use/runs/run_abc123" \
  -H "X-Browser-Use-API-Key: $BROWSER_USE_API_KEY"
# → { "id": "run_abc123", "browser_id": "br_...", "status": "completed", ... }
```
