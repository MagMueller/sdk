...
| ----- | ---------- | ----: | ---------: | -----: | ---- |
| GPT-5.6 Luna (recommended, default) | `gpt-5.6-luna` | \$0.24 | \$0.024 | \$1.44 | OpenAI |
...
<Tip>
  ...
  current V4 production traffic. Use **Claude Opus 5** when maximum accuracy
  matters most.
</Tip>
...
See [Thinking levels](/cloud/agent/thinking-levels) for the reasoning values
accepted by each model. Not every model supports configurable thinking.

<Note>
  New model strings can reach the API before the TypeScript SDK's generated
  union. If TypeScript rejects a model listed on this page, call
  `POST /api/v4/runs` directly for that model.
</Note>
